...
# The full list of gene symbols you provided
gene_symbols = [
...
# Query the database for the list of genes, requesting 'name' and 'summary'
# The 'fields' parameter specifies the annotations to retrieve.
gene_annotations = mg.querymany(gene_symbols, scopes='symbol', fields='name,summary', species='human')

# Convert the results to a pandas DataFrame
df = pd.DataFrame(gene_annotations)

# Select relevant columns and clean up
df = df[['query', 'name', 'summary']]
df.rename(columns={'query': 'gene_symbol'}, inplace=True)

# Save the DataFrame to a CSV file
df.to_csv("gene_annotations.csv", index=False)

print("Annotations have been successfully saved to gene_annotations.csv")


gene_info = mg.query('RPL11', fields='go')

# The GO terms are structured with a 'term' field for the description
# and an 'id' for the GO accession number
if gene_info['hits']:
    go_terms = gene_info['hits'][0]['go']
    print("GO terms for RPL11:")
    for term in go_terms:
        print(f"  - {term['term']} (GO:{term['id']})")