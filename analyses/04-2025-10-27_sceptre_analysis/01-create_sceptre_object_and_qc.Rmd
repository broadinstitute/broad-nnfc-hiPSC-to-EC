...
# Plot the number of guides per element in the discovery pairs
grna_per_element <- table(grna_to_element_map$grna_target)
hist(grna_per_element[discovery_pairs$V1], breaks = 50, main = "Number of gRNAs per element", xlab = "Number of gRNAs per element")
...
tmp = guide_metadata$aliases[match(discovery_pairs$grna_id, guide_metadata$guide_id)]
grna_to_element_with_non_targeting_map$grna_id = tmp
head(grna_to_element_with_non_targeting_map)
```
...
p <- ggplot(data.frame(num_cells = element_grna_counts), aes(x = num_cells)) +
  ...
  geom_vline(xintercept = median(element_grna_counts), color = "red", linetype = "dashed", size = 1) +
...
write_outputs_to_directory(
  sceptre_object = sceptre_object,
  directory = paste0(output_folder,"sceptre_outputs_day",selected_day)
)
```
...
```{r}
saveRDS(sceptre_object, file = paste0(output_folder,"sceptre_object_day",selected_day,".rds"))
```
