...
```{r}
# INPUTS
metadata_samples_fnp <- "../../metadata/hiPSC-EC/sample_metadata_endothelial_terra.tsv"
metadata_genes_fnp <- "../../metadata/hiPSC-EC/metadata_genes_corrected_names.csv"
metadata_guides_fnp <- "../../metadata/hiPSC-EC/metadata_guides_with_accession.csv"
...
negative_controls_fnp <- "../../results/2025-09-15/sceptre_non_targeting_guides_hg38.tsv"
...
h5_files_fnp <- list.files(path = "../../data/hiPSC-EC/h5", pattern = "*.h5", full.names = TRUE, recursive = TRUE)
...
Seurat::FeatureScatter(
  ...
  scale_color_manual(name="Replicate", values = palette_4_qual) +
  xlab("Total UMIs") +
  ylab("UMIs in readout genes") +
  theme(
    plot.title = element_text(hjust = 0.5),
...
## Filter cells
Using percentage of UMI from target genes >= 50%, percentage of ribosomal UMIs>=20, and number of UMIs >= 1000

```{r}
d0_data_filtered = subset(d0_data, subset = percent_target_umis >= 50 & nCount_RNA >= 1000 & percent.ribo > 20)
d0_data_filtered
...
SeuratObject::SaveSeuratRds(
  ...
  file = paste0(output_folder,"/seurat_object_filtered_norm.rds")
  )
```