...
def _(mo):
    mo.md(r"""
    This notebook reads the results of the primer alignment to transcript FASTA file and check that the gene we associated with the primer is indeed present in the transcript FASTA file.
    """)
    return
...
def _(mo):
    mo.md(r"""
    Load the raw gene metadata file.
    """)
    return
...
def _(mo):
    mo.md(r"""
    Load the primer mapping results.
    """)
    return
...
def _(mo):
    mo.md(r"""
    Create a gene symbol to gene ID mapping using the results of the primer mapping.
    """)
    return
...
def _(mo):
    mo.md(r"""
    List the intended target genes
    """)
    return
...
def _(mo):
    mo.md(r"""
    Load the central and locus genes annotations. I am also updating the gene names if needed and save the new files.
    """)
    return
...
def _(gene_name_update_dict, pd):
    locus_annotations = pd.read_csv("../../metadata/legacy/ec_screen_16_loci_2mb_info_raw.tsv", sep="\t", header=0)
    locus_bed = pd.read_csv("../../metadata/legacy/ec_screen_16_loci_2mb_locus_raw.bed", sep="\t", header=None, names=["chrom", "start", "end", "gene_symbol"])
...
def _(mo):
    mo.md(r"""
    For each gene in `central_gene` and `gene_names` columns create a mapping to a tuple (is_central_gene: bool, locus_central_gene: gene_symbol).
    """)
    return
...
def _(mo):
    mo.md(r"""
    Add annotations about:
    - Day 0 marker
    - Day 2 marker
    - Day 4 marker
    - housekeeping gene
    - involved in positive control pair

    Tha annotations comes from the file `../../metadata/legacy/all_genes_for_primer_design_w_tpm_legacy.tsv`.
    """)
    return
...
def _(gene_name_update_dict, pd):
    ...
    extra_annotations = pd.read_csv("../../metadata/legacy/all_genes_for_primer_design_w_tpm_legacy.tsv", sep="\t", header=0)
    extra_annotations

    # The gene_name needs to be updated with the results of the primer mapping
    extra_annotations["updated_gene_name"] = extra_annotations["gene_name"].replace(gene_name_update_dict)
    extra_annotations

    # Using the column `type`, create boolean columns for each of the following annotations:
    annotation_types = list(set(extra_annotations["type"]))
    for annotation in annotation_types:
        extra_annotations[annotation.replace(" ", "_").lower()] = extra_annotations["type"].apply(lambda x: annotation in x)
    extra_annotations
...
def _(mo):
    ...
    Columns:
     ...
     - is_d0_marker: Is Day 0 marker (True/False)
     - is_d2_marker: Is Day 2 marker (True/False)
     - is_d4_marker: Is Day 4 marker (True/False)
     - is_housekeeping_gene: Is housekeeping gene (True/False)
     - is_in_positive_control_pair: Is involved in positive control pair (True/False)
...
def _(
    ...
    # Add the Day 0 marker, Day 2 marker, Day 4 marker, housekeeping gene, and positive control pair annotations
    annotation_columns = {
        "is_d0_marker": "d0_marker",
        "is_d2_marker": "d2_marker",
        "is_d4_marker": "d4_marker",
        "is_housekeeping_gene": "p2_housekeeping",
        "is_in_positive_control_pair": "positive_control"
    }
    for new_col, source_col in annotation_columns.items():
        annotation_dict = dict(zip(extra_annotations["updated_gene_name"], extra_annotations[source_col]))
        metadata_final[new_col] = metadata_final["intended_target_gene_symbol"].apply(
            lambda x: annotation_dict.get(x, False)
        )
    metadata_final
    ...
    metadata_final = metadata_final[
        [
            ...
            "outer_primer",
            "inner_primer",
            "inner_primer_adapter",
            "is_central_gene",
            "locus_central_gene",
            "is_d0_marker",
            "is_d2_marker",
            "is_d4_marker",
            "is_housekeeping_gene",
            "is_in_positive_control_pair",
            "well",
            "batch"
    ]]
    ...
    metadata_final.to_csv("../../results/2025-10-08_create_gene_metadata/metadata_genes_final.csv", index=False)
