---
title: "d0-hiPSC-HPC-Seurat"
output: html_document
editor_options: 
  chunk_output_type: console
---

# Day 0 of hiPSC-HPC differentiation, analyzed with SCEPTRE
...
SCEPTRE required as inputs:
 - count files: Seurat RDS object generated during this `../2025-09-18/d0-hiPSC-HPC-load-counts-into-Seurat.Rmd` analysis.
...
```{r}
output_folder <- "results/2025-09-19/"
dir.create(output_folder, showWarnings = FALSE, recursive = TRUE)

grna_to_element_map_fnp <- "results/2025-09-15/grna_to_element_mapping_hg38.tsv"
seurat_rds_fnp <- "results/2025-09-18/d0_seurat_object_filtered_norm.rds"
...
non_targeting_fnp <- "results/2025-09-15/grna_non_targeting_input.tsv"
...
positive_control_pairs <- read.delim(positive_control_pairs_fnp, stringsAsFactors = FALSE)
message("Number of positive control pairs: ", nrow(positive_control_pairs))
discovery_pairs <- read.delim(discovery_pairs_fnp, stringsAsFactors = FALSE)
message("Number of discovery pairs: ", nrow(discovery_pairs))
non_targeting <- read.delim(non_targeting_fnp, stringsAsFactors = FALSE)
...
discovery_pairs$response_id <- gene_map$gene_symbol[match(discovery_pairs$response_id, gene_map$gene_id)]
# Change the positive control pairs response_id column to gene_symbols
positive_control_pairs$response_id <- gene_map$gene_symbol[match(positive_control_pairs$response_id, gene_map$gene_id)]
...
grna_counts <- GetAssayData(d0_seurat_object, assay = "gRNA", layer = "counts")
...
present <- present[present %in% gene_map$gene_id]
length(present)

rna_counts_readout_only <- rna_counts[gene_map$gene_symbol[match(present, gene_map$gene_id)], , drop = FALSE]
message("Dimensions of RNA count matrix (subset to readout genes): ", paste(dim(rna_counts_readout_only), collapse = " x "))

grna_counts <- grna_counts[grna_to_element_with_non_targeting_map$grna_id, , drop = FALSE]
...
```{r}
sceptre_object <- run_qc(sceptre_object, p_mito_threshold = 1)
...
```{r}
result <- get_result(
  sceptre_object = scqeptre_object,
