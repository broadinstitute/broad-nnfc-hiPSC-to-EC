...
binned_power <- binned_power %>%
  dplyr::mutate(
    effect_size_q = cut(
      effect_size,
      breaks = quantile(effect_size, probs = seq(0, 1, length.out = 6), na.rm = TRUE),
      include.lowest = TRUE
    )
  )
...
ggplot(
  binned_power,
  aes(x = mean_expr, y = mean_power, color = effect_size_q, group = effect_size_q)
) +
  ...
  facet_wrap(~ cells_quant + effect_size_q) +
  labs(
