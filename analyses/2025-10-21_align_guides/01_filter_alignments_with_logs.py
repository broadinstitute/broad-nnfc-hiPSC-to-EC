...
def has_indels_in_region(cigar_string: str, query_start: int, query_end: int) -> bool:
    ...
    for length, op in cigar_ops:
        if op in QUERY_CONSUMING_OPS:
            ...
            if op == 'I':
                # Insertion: check if the insertion position is within our region
                if query_start <= query_pos < query_end:
                    return True
            
            # Advance query position for all query-consuming operations
            query_pos += length
            
        elif op in DELETION_SKIP_OPS:
            # Deletion: check if we're currently in the region of interest
            # Deletions don't consume query sequence, so they occur "between" query positions
            if query_start <= query_pos < query_end:
                return True
...
def process_alignment_record(
    ...
    # New primary filters
    if nm > 1:
        reason = "discarded_nunmber_mismatched_gt_1"
    elif as_score < 17:
        reason = "discarded_alignment_score_lt_17"
    else:
        # Check for indels in protospacer region if emitting protospacer
        if args.emit_protospacer:
            if is_reverse_strand:
                # Minus strand: PAM at beginning, protospacer after PAM
                skip_leading_g = 1 if args.drop_leading_g else 0
                query_start = PAM_LENGTH + skip_leading_g  # Skip PAM + optional G
                query_end = query_length  # Go to end of sequence
            else:
                # Plus strand: PAM at end, protospacer before PAM
                query_start = 1 if args.drop_leading_g else 0
                query_end = query_length - MIN_PAM_CLEAN_BASES  # Exclude last 2 bases (NG from NGG PAM)
            
            if has_indels_in_region(cigar, query_start, query_end):
                reason = "discarded_protospacer_indel"
        
        # Only check PAM cleanliness if we haven't already failed other filters
        if reason == "kept" and args.require_pam_clean and not is_pam_region_clean(query_to_ref_map, query_length, mismatch_indices, is_reverse_strand):
    ...
    # Update statistics and return early if discarded
    if reason != "kept":
        stats.total_discarded += 1
        return None, (read_name, chrom, ref_start_1b, aln.flag, mapq, strand,
                      cigar, nm, as_score, md or '', reason)
